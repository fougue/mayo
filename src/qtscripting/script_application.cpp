...
ScriptApplication::ScriptApplication(const ApplicationPtr& app, ScriptMayo* mayoObject)
    : QObject(mayoObject),
      m_app(app),
      m_scriptMayo(mayoObject)
{
    if (app) {
        m_sigConns
...
Ptr_ScriptDocument ScriptApplication::documentAt(int docIndex) const
{
...
Ptr_ScriptDocument ScriptApplication::findDocumentByLocation(QString location) const
{
    if (!m_app)
        return nullptr;
...
int ScriptApplication::findIndexOfDocument(Ptr_ScriptDocument doc) const
{
...
void ScriptApplication::closeDocument(Ptr_ScriptDocument doc)
{
    auto jsDoc = qobject_cast<ScriptDocument*>(doc);
    if (m_app && jsDoc) {
